...
在跳转到 start_of_setup，grub 等引导程序已经将 `fs = es = ds = ss = 0x1000` 并且 `cs = 0x1020`。
...
就像我在上面一节中所写的， 为了能够跳转到 `_start` 标号出执行代码，grub2 将 `cs` 段寄存器的值设置成了 `0x1020`，这个值和其他段寄存器都是不一样的，因此下面的代码就是**将 `cs` 段寄存器的值和其他段寄存器一致**：
...
此时，`cs=ds=es=0x1000`，并且跳转到 6f 开始执行。
...
```assembly
	movw	%ss, %dx
	cmpw	%ax, %dx				# 此时 ax=ds=0x1000，%ds == %ss?
	movw	%sp, %dx
	je	2f							# 如果 ds == ss，跳转
	......							# 如果 ds != ss
```
...
* 第一种情况，`ss` 寄存器的值是 0x10000，在这种情况下，代码将直接跳转到标号为 `2` 的代码处执行:
...
CAN_USE_HEAP 设置为 1 表示 heap_end_ptr 中输入的值有效。如果清除此字段，某些设置代码功能将被禁用。



`loadflags` 字段其他可以设置的标志包括(`arch/x86/include/uapi/asm/bootparam.h`)：

...
官方提供的方法是我们可以定义它的别名函数来代替，如上所示weak_ref的别名为test，所以在main.c中定义了test函数就相当于定义了weak_ref函数，所以在test\_func的判断分支中，test_func中不为null，执行if(test_func)分支，如果在main.c中去除weak_ref的定义，函数的执行结果是这样的:
